...
function generateSliderCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}self.${safeId} = ctk.CTkSlider(self, width=${props.width || 160}, height=${props.height || 16}, corner_radius=${props.cornerRadius}, border_width=${props.borderWidth}, button_color="${props.fg_color || '#3b82f6'}", button_hover_color="${adjustColorBrightness(props.fg_color || '#3b82f6', -20)}", progress_color="${props.fg_color || '#3b82f6'}")\n`;
  code += `${indent}self.${safeId}.place(x=${props.x}, y=${props.y})\n`;
...
function generateProgressBarCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}self.${safeId} = ctk.CTkProgressBar(self, width=${props.width || 160}, height=${props.height || 16}, corner_radius=${props.cornerRadius}, border_width=${props.borderWidth}, progress_color="${props.fg_color || '#3b82f6'}")\n`;
...
function generateCheckboxCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}self.${safeId} = ctk.CTkCheckBox(self, text="${props.text || 'Checkbox'}", ${props.fontConfig}, fg_color="${props.fg_color || '#3b82f6'}", hover_color="${adjustColorBrightness(props.fg_color || '#3b82f6', -20)}", text_color="${props.text_color || '#ffffff'}")\n`;
  code += `${indent}self.${safeId}.place(x=${props.x}, y=${props.y})\n`;
...
function generateNotebookCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}# TabView (CTk notebook equivalent)\n`;
  code += `${indent}self.${safeId} = ctk.CTkTabview(self, width=${props.width || 300}, height=${props.height || 200}, corner_radius=${props.cornerRadius}, fg_color="${props.fg_color || '#2b2b2b'}", border_width=${props.borderWidth}, border_color="${props.borderColor}", segmented_button_fg_color="${props.fg_color || '#2b2b2b'}", segmented_button_selected_color="${props.fg_color || '#3b82f6'}", text_color="${props.text_color || '#ffffff'}")\n`;
  code += `${indent}self.${safeId}.place(x=${props.x}, y=${props.y})\n`;
  code += `${indent}\n`;
  code += `${indent}# Add tabs\n`;
  code += `${indent}tab_titles = "${props.tabs || 'Tab 1,Tab 2,Tab 3'}".split(',')\n`;
  code += `${indent}for tab_title in tab_titles:\n`;
  code += `${indent}    self.${safeId}.add(tab_title.strip())\n`;
...
function generateCanvasCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}self.${safeId}_frame.place(x=${props.x}, y=${props.y})\n`;
  code += `${indent}self.${safeId} = tk.Canvas(self.${safeId}_frame, width=${props.width - 4 || 196}, height=${props.height - 4 || 146}, bg="${adjustColorHex(props.fg_color || '#2b2b2b')}", highlightthickness=0)\n`;
...
function generateDatePickerCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}    self.${safeId}_frame.place(x=${props.x}, y=${props.y})\n`;
  code += `${indent}    # Use tkcalendar's DateEntry\n`;
  code += `${indent}    self.${safeId} = tkcalendar.DateEntry(self.${safeId}_frame, width=12, background="${adjustColorHex(props.fg_color || '#3b82f6')}", foreground="${adjustColorHex(props.text_color || '#ffffff')}", borderwidth=0)\n`;
  code += `${indent}    self.${safeId}.place(relx=0.5, rely=0.5, anchor="center")\n`;
  code += `${indent}except ImportError:\n`;
  code += `${indent}    print("tkcalendar not installed. Install with: pip install tkcalendar")\n`;
  code += `${indent}    # Fallback to a button\n`;
  code += `${indent}    self.${safeId} = ctk.CTkButton(self, text="Date Picker", width=${props.width}, height=${props.height}, corner_radius=${props.cornerRadius}, fg_color="${props.fg_color || '#3b82f6'}", text_color="${props.text_color || '#ffffff'}", ${props.fontConfig})\n`;
  code += `${indent}    self.${safeId}.place(x=${props.x}, y=${props.y})\n`;
...
function generateRadioButtonCode(safeId: string, props: any, indent: string): string {
  ...
  code += `${indent}self.${safeId} = ctk.CTkRadioButton(self, text="${props.text || 'Radio Button'}", fg_color="${props.fg_color || '#3b82f6'}", hover_color="${adjustColorBrightness(props.fg_color || '#3b82f6', -20)}", text_color="${props.text_color || '#ffffff'}", variable=self.${safeId}_var, value="selected", ${props.fontConfig})\n`;
  code += `${indent}self.${safeId}.place(x=${props.x}, y=${props.y})\n`;
...
// Helper function to convert to tkinter hex format
function adjustColorHex(hexColor: string): string {
  if (!hexColor || hexColor === 'transparent') {
    return '#2b2b2b';  // Default dark color
  }
